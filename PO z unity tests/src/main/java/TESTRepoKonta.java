...
public class TESTRepoKonta implements Repository {
    public int addKontoOsobiste(String nrPesel, Double saldo, Double limitDebetu) {
        ...
        try (Connection conn = TESTConnectorBazaKont.getConnection();
            ...
            System.out.println("[TEST]Konto osobiste dodane!");
            
        } catch (SQLException e) {
            System.out.println("[TEST]Błąd podczas dodawania: " + e.getMessage());
        }
        return -1;
    }
    public int addKontoOszczednosciowe(String nrPesel, Double saldo) {
        ...
        try (Connection conn = TESTConnectorBazaKont.getConnection();
            ...
            System.out.println("[TEST]Konto osczednosciowe dodane!");
    ...
    try (Connection conn = TESTConnectorBazaKont.getConnection();
        ...
        if (rowsAffected > 0) {
            System.out.println("[TEST]Baza danych zaktualizowana (numer Konta: " + idKonta + ").");
        }
        
    } catch (SQLException e) {
        System.err.println("[TEST]Błąd aktualizacji salda: " + e.getMessage());
    }
    }
    public void wyswietlKontaKlienta(String peselKlienta) {
        ...
        try (Connection conn = TESTConnectorBazaKont.getConnection();
            ...
            while(rs.next()) {
                ...
                System.out.println("[TEST]Konto" + typ + ": Numer konta: " + id + " | Saldo: " + saldo + " PLN");
            }
            
            if(!znaleziono) {
                System.out.println("[TEST]Brak otwartych kont dla numeru PESEL: " + peselKlienta);
            }
            
        } catch (SQLException e) {
            System.out.println("[TEST]Błąd pobierania kont: " + e.getMessage());
        }
    }
    public Double getSaldo(int nrKonta){
        ...
        System.out.println("[TEST]Błąd pobierania salda " + e.getMessage());
        }
        return saldo;
    }
    ...
    public Konto zaladujKonto(int nrKonta, String nrPesel) {
        ...
        try (Connection conn = TESTConnectorBazaKont.getConnection();
            ...
            System.out.println("[TEST]Błąd ladowania konta " + e.getMessage());
        }
        return null;
    }
}
