...
public class TESTRepoKlienci {

    // Rejestracja nowego Klienta
    public void dodajKlienta(Klient k) {
        String sql = "INSERT INTO klienci(imie, nazwisko, pesel, email, haslo) VALUES(?, ?, ?, ?, ?)";

        try (Connection conn = ConnectorBazaKlientow.getConnection();
            ...
            pstmt.executeUpdate();
            System.out.println("[TEST]Zarejestrowano pomyślnie klienta: " + k.imie + " " + k.nazwisko);
            
        } catch (SQLException e) {
            System.out.println("[TEST]Błąd rejestracji (może taki PESEL już istnieje?): " + e.getMessage());
        }
    }
    ...
    public Klient zaloguj(String pesel, String haslo) {
        ...
        try (Connection conn = ConnectorBazaKlientow.getConnection();
            ...
            System.out.println("[TEST]Błąd logowania: " + e.getMessage());
        }
